...
SW.GetClassCodeView = SW.ModalContentView.extend({
	...
	// Overrides default
	render: function(){
		var template = this.findTemplate('class-code-username');
...
SW.LoginForClassCodeView = SW.ModalContentView.extend({
	...
	// Overrides default
	render: function(){
		var template = this.findTemplate('class-code-login');
		var context = this.model.toJSON();
		html = template(context);
...
SW.RegisterForClassCodeView = SW.ModalContentView.extend({
	...
	// Overrides default
	render: function(){
		var template = this.findTemplate('class-code-register');
		var context = this.model.toJSON();
		html = template(context);
...
SW.DisplayClassCodeView = SW.ModalContentView.extend({
	...
	// Overrides default
	render: function(){
		var template = this.findTemplate('class-code-display');
		var context = this.model.toJSON();
		html = template(context);
...
SW.AppleInstallPromptView = SW.ParentView.extend({
	...
	openAppStore: function() {
		...
		location.href = "http://web.archive.org/web/20200402165647/https://geo.itunes.apple.com/ca/app/prodigy-math-game/id950795722?mt=8";
	},
...
SW.ToyUnlock = SW.Model.extend({
	...
	submit: function(studentID, token, uniqueCode) {
		var validCode = uniqueCode.replace(/0/g,'O'); //covert 0's (zeros) to Oh's
		console.log("submitting", studentID, token, validCode);
